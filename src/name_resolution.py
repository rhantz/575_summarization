...
def generate_output(file_path, topic_dir):
    ...
    with open(file_path, 'r') as f:
        pos_CR = ''  # initiate an empty output string
        names = set()
        for sent in f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--preCR_dir", type=str, required=True, help="path to the directory that stores summaries generated after CS and IO")
    parser.add_argument(
        "--source_dir", type=str, required=True, help="path to the directory that stores pre-processed article before CS")
    parser.add_argument(
        "--postCR_dir", type=str, required=True, help="path to the directory that stores summaries generated after CS, IO, and CR")

    args = parser.parse_args(sys.argv[1:])

    nlp = get_nlp()
    summaries = sorted([f for f in listdir(args.preCR_dir) if isfile(join(args.preCR_dir, f))])
    for sum in summaries:
        if not sum.startswith('.'):  #ignore hidden '.DS_Store' file in MacOS
            file_path = join(args.preCR_dir, sum)
            topic_id = sum.split('-')[0]  # 'D1001' referring original data
            topic_dir = track_topic(args.source_dir, topic_id)
            # print('\n\n' + topic_dir)
            pos_CR = generate_output(file_path, topic_dir)

            # create D5 dir and output files
            if not exists(args.postCR_dir):
                makedirs(args.postCR_dir)
            D5_output = join(args.postCR_dir, sum)
